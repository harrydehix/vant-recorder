...
import RecorderSettings, {
    ...
    defaultCurrentConditionsTaskSettings,
    defaultRecorderSettings,
} from "./settings/RecorderSettings";
import merge from "lodash.merge";
import MinimumRecorderSettings, {
    MinimumCurrentConditionsTaskSettings,
} from "./settings/MinimumRecorderSettings";
...
class Recorder {
    ...
    public readonly station: WeatherStationAdvanced;
    ...
    public static create = async (
        recorderSettings: MinimumRecorderSettings
    ) => {
    ...
    private static async createDeviceInterface(settings: RecorderSettings) {
        log.info(`Connecting to device ${settings.path}...`);
        return await WeatherStationAdvanced.connect(settings);
    }
    ...
    private static loadEnvironmentVariablesAndConfigureLogger(
        ...
        if (
            process.env.API &&
            validator.isURL(process.env.API, { require_tld: false })
        ) {
        ...
        if (
            ...
            validator.isIn(process.env.BAUD_RATE, BaudRates)
        ) {
        ...
        if (
            ...
            validator.isIn(process.env.LOG_LEVEL, [
                "debug",
                "info",
                "warn",
                "error",
        ...
        if (
            process.env.RAIN_COLLECTOR_SIZE &&
            validator.isIn(process.env.RAIN_COLLECTOR_SIZE, RainCollectorSizes)
        ) {
        ...
        if (
            ...
            validator.isBoolean(process.env.CONSOLE_LOG)
        ) {
        ...
        if (
            process.env.LOG_ERROR_INFORMATION &&
            validator.isBoolean(process.env.LOG_ERROR_INFORMATION)
        ) {
            settings.logOptions.logErrorInformation =
                process.env.LOG_ERROR_INFORMATION === "true";
        } else {
            invalidEnvironmentVariables.push("LOG_ERROR_INFORMATION");
        }
        if (
            process.env.RAIN_UNIT &&
            validator.isIn(process.env.RAIN_UNIT, RainUnits)
        ) {
        ...
        if (
            ...
            validator.isIn(process.env.TEMPERATURE_UNIT, TemperatureUnits)
        ) {
            settings.units!.temperature = process.env
        ...
        if (
            process.env.PRESSURE_UNIT &&
            validator.isIn(process.env.PRESSURE_UNIT, PressureUnits)
        ) {
            settings.units!.pressure = process.env
                .PRESSURE_UNIT as PressureUnit;
        } else {
            invalidEnvironmentVariables.push("PRESSURE_UNIT");
        }

        if (
            process.env.SOLAR_RADIATION_UNIT &&
            validator.isIn(
                process.env.SOLAR_RADIATION_UNIT,
                SolarRadiationUnits
            )
        ) {
            settings.units!.solarRadiation = process.env
                .SOLAR_RADIATION_UNIT as SolarRadiationUnit;
        } else {
            invalidEnvironmentVariables.push("SOLAR_RADIATION_UNIT");
        }

        if (
            process.env.WIND_UNIT &&
            validator.isIn(process.env.WIND_UNIT, WindUnits)
        ) {
        ...
        for (const invalidEnvironmentVariable of invalidEnvironmentVariables) {
            log.warn(
                `Invalid or missing environment variable '${invalidEnvironmentVariable}'!`
            );
        }
    ...
    private static validateSettings(settings: RecorderSettings) {
        if (!settings.path) {
            ...
            throw new InvalidRecorderConfigurationError(
                "No serial path specified!"
            );
        }
        ...
        if (!settings.rainCollectorSize) {
            ...
            throw new InvalidRecorderConfigurationError(
                "No rain collector size specified!"
            );
        }
     ...
     * @throws {@link InvalidRecorderConfigurationError} if the settings are invalid
     */
    public configureCurrentConditionsTask = (
        settings: MinimumCurrentConditionsTaskSettings | false
    ) => {
        if (!settings) {
            this.currentConditionsTaskSettings = undefined;
        } else {
            this.currentConditionsTaskSettings = merge(
                defaultCurrentConditionsTaskSettings,
                settings
            );
            ...
            if (settings.preferEnvironmentVariables) {
                ...
                if (interval && validator.isInt(interval, { min: 1 })) {
                    this.currentConditionsTaskSettings.interval =
                        parseInt(interval);
                } else {
                    invalidEnvironmentVariables.push(
                        "CURRENT_CONDITIONS_INTERVAL"
                    );
                }
            }
            ...
            if (
                !this.currentConditionsTaskSettings?.interval ||
                this.currentConditionsTaskSettings?.interval < 1
            ) {
                throw new InvalidRecorderConfigurationError(
                    "The current conditions interval has to be greater or equal to 1."
                );
            }
        }
    };
     ...
     */
    public currentConditionsConfigured = () =>
        this.currentConditionsTaskSettings !== undefined;
     ...
     */
    public currentConditionsInterval = () =>
        this.currentConditionsTaskSettings?.interval;
    ...
    protected updateCurrentConditions = async () => {
        ...
        let [record, err] = await this.station.getDetailedRealtimeData();
        if (err) {
            log.error(
                "Error while getting detailed realtime data from the station!"
            );
            log.error(err);
        }
        ...
        superagent
            ...
            .end((err, res: superagent.Response) => {
                if (!res || !res.ok) {
                    log.error(
                        `Failed to send realtime record to '${this.settings.api}'!`
                    );
